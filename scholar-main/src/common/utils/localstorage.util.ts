...
import { defaultCustomColumn } from '@/common/constants'

export class LocalStorageUtil {
  static putCustomColumnValue (data) {
    const key = ToolsUtil.createCacheKey('customColumn_VUE')
    localStorage.setItem(key, JSON.stringify(data))
  }

  static getCustomColumnValue () {
    const key = ToolsUtil.createCacheKey('customColumn_VUE')
    const customColumnValue = localStorage.getItem(key)
    if (customColumnValue) {
      return JSON.parse(customColumnValue)
    }
    return defaultCustomColumn
  }

  static removeCustomColumnValue () {
    const key = ToolsUtil.createCacheKey('customColumn_VUE')
    return localStorage.removeItem(key)
  }

  static getTheme () {
    return localStorage.getItem(ToolsUtil.createCacheKey('THEME_VUE'))
  }

  static putTheme (theme): void {
    localStorage.setItem(ToolsUtil.createCacheKey('THEME_VUE'), theme)
  }

  static removeTheme () {
    localStorage.removeItem(ToolsUtil.createCacheKey('THEME_VUE'))
  }

  // 设置管理员信息
  static putAdminUser (userInfo: Json): void {
    localStorage.removeItem('SCHOLAR_ADMIN_INFO_VUE_' + ToolsUtil.getOrgCode())
    if (userInfo) {
      localStorage.setItem('SCHOLAR_ADMIN_INFO_VUE_' + ToolsUtil.getOrgCode(), JSON.stringify(codeJsonAES(userInfo)))
    }
  }

  // 因从订单中获取渠道，所以不能用渠道拼接命名（存在因果关系）
  static setClassMarketData (value: Json) {
    const key = 'MarketData_VUE'
    localStorage.setItem(key, JSON.stringify(value))
  }

  static getAdminUser (): any {
    const str = localStorage.getItem('SCHOLAR_ADMIN_INFO_VUE_' + ToolsUtil.getOrgCode())
    let userInfo: Json = {}
    if (str) {
  ...
  static putCollegeLib (info: Json): void {
    localStorage.removeItem('SCHOLAR_CollegeLib_VUE_' + ToolsUtil.getOrgCode())
    if (info) {
  ...
  static getCollegeLib (): any {
    const str = localStorage.getItem('SCHOLAR_CollegeLib_VUE_' + ToolsUtil.getOrgCode())
    let info: Json = {}
    if (str) {
      info = JSON.parse(str)
    }
  ...
  static putSavePath (info: Json): void {
    localStorage.removeItem('SCHOLAR_SAVE_PATH_VUE' + ToolsUtil.getOrgCode())
    if (info) {
      localStorage.setItem('SCHOLAR_SAVE_PATH_VUE' + ToolsUtil.getOrgCode(), JSON.stringify(info))
    }
  }

  static getSavePath (): any {
    const str = localStorage.getItem('SCHOLAR_SAVE_PATH_VUE' + ToolsUtil.getOrgCode())
    let info: Json = {}
    if (str) {
      info = JSON.parse(str)
    }
  ...
  static putBaseParams (baseParams: Json): void {
    const key = ToolsUtil.createCacheKey('BaseParams_VUE_')
    if (baseParams) {
  ...
  static getDefaultTask () {
    const key = ToolsUtil.createCacheKey('TASK_ID_VUE')
  ...
  static putAnswersInfo (answersInfo: Json): void {
    if (answersInfo) {
      const key = ToolsUtil.createCacheKey('ANSWERS_INFO_VUE')
  ...
  static getCaterysTree () {
    ...
    const key = ToolsUtil.createCacheKey('caterysTree_VUE')
    const str = localStorage.getItem(key)
    if (str) {
  ...
  static getBackUrl () {
    const key = ToolsUtil.createCacheKey('BACK_URL_VUE')
  ...
  static setExerciseKey (
    ...
    const key = ToolsUtil.createCacheKey('ExerciseKeyData_VUE_')
  ...
  static putOfficerUser (userInfo: Json): void {
    localStorage.removeItem('JW_USER_INFO_' + ToolsUtil.getOrgCode())
    if (userInfo) {
      localStorage.setItem('JW_USER_INFO_' + ToolsUtil.getOrgCode(), JSON.stringify(codeJsonAES(userInfo)))
    }
  }

  static putOperatorUser (userInfo: Json): void {
    localStorage.removeItem('OPERATOR_USER_INFO_' + ToolsUtil.getOrgCode())
    if (userInfo) {
      localStorage.setItem('OPERATOR_USER_INFO_' + ToolsUtil.getOrgCode(), JSON.stringify(codeJsonAES(userInfo)))
    }
  }

  static putTeacherUser (userInfo: Json): void {
    localStorage.removeItem('TCH_USER_INFO_' + ToolsUtil.getOrgCode())
    if (userInfo) {
      localStorage.setItem('TCH_USER_INFO_' + ToolsUtil.getOrgCode(), JSON.stringify(codeJsonAES(userInfo)))
    }
  }
  ...
  static putCodeUid (codeUid: string): void {
    localStorage.setItem('SCHOLAR_CODEUID_VUE_' + ToolsUtil.getOrgCode(), codeUid)
  }

  static getCodeUid () {
    return localStorage.getItem('SCHOLAR_CODEUID_VUE_' + ToolsUtil.getOrgCode())
  }

  static removeCodeUid () {
    localStorage.removeItem('SCHOLAR_CODEUID_VUE_' + ToolsUtil.getOrgCode())
  }
  ...
  static removePracSubquestionbank (): void {
    localStorage.removeItem('SELECTD_PRAC_SUBQUESTIONBANK_VUE_' + ToolsUtil.getOrgCode())
  }

  static putPracSubquestionbank (name: string): void {
    localStorage.setItem('SELECTD_PRAC_SUBQUESTIONBANK_VUE_' + ToolsUtil.getOrgCode(), name)
  }

  static getPracSubquestionbank () {
    const str = localStorage.getItem('SELECTD_PRAC_SUBQUESTIONBANK_VUE_' + ToolsUtil.getOrgCode())
    return str
  }

  static removeExamSubquestionbank (): void {
    localStorage.removeItem('SELECTD_EXAM_SUBQUESTIONBANK_VUE_' + ToolsUtil.getOrgCode())
  }

  static putExamSubquestionbank (name: string): void {
    localStorage.setItem('SELECTD_EXAM_SUBQUESTIONBANK_VUE_' + ToolsUtil.getOrgCode(), name)
  }

  static getExamSubquestionbank () {
    const str = localStorage.getItem('SELECTD_EXAM_SUBQUESTIONBANK_VUE_' + ToolsUtil.getOrgCode())
    return str
  }

  static clearAll (): void {
    LocalStorageUtil.removeCustomColumnValue()
